...
export default function Landing() {
  ...
  return (
    <div style={{ position: "relative", zIndex: 2, width: "100%", overflowX: "hidden" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {[
            ...
            <a key={l.label} href={l.href} className="desktop-only" style={{ fontSize: 11, color: "var(--muted)", textDecoration: "none", fontFamily: "Syne, sans-serif", letterSpacing: "0.06em", textTransform: "uppercase", transition: "color 0.2s", padding: "0 12px" }}
      ...
      {/* HOW IT WORKS */}
      <section id="how" style={{ borderTop: "1px solid var(--border)", padding: "clamp(56px,10vw,112px) clamp(16px,4vw,48px)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <FadeUp>
            <p className="number-label" style={{ color: "var(--accent)", marginBottom: 16 }}>How It Works</p>
            <h2 style={{ fontSize: "clamp(28px,5vw,56px)", fontWeight: 800, lineHeight: 1.05, marginBottom: 16, letterSpacing: "-0.03em" }}>Three users. One protocol.</h2>
            <p style={{ fontSize: "clamp(13px,2vw,16px)", color: "var(--muted)", maxWidth: 480, lineHeight: 1.7, marginBottom: 48 }}>
          ...
          <div style={{ display: "flex", gap: 1, background: "var(--border)", marginBottom: 1 }}>
            {[
              ...
              <button key={f.label} onClick={() => setActiveFlow(i)} style={{
                ...
                border: "none", cursor: "pointer", padding: "clamp(14px,2vw,18px) 12px",
                fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: "clamp(11px,2vw,13px)",
          ...
          {[
            {
              ...
              steps: [
                ...
                { n: "03", title: "Receive Credentials", body: "Verified institutions issue soulbound NFT credentials directly to your wallet. Cannot be transferred or faked." },
                { n: "04", title: "Borrow USDC", body: "Score 250+ unlocks a credit limit. Borrow USDC with no collateral. Repay on time to grow your limit." },
              ],
            },
            {
            ...
            },
            {
              ...
              steps: [
                ...
                { n: "03", title: "Instant Verification", body: "See their full credential portfolio, issuer trust levels, and reputation score breakdown." },
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 1, background: "var(--border)" }}>
                {flow.steps.map((s, i) => (
                  <FadeUp key={s.n} delay={i * 0.08}>
                    <div className="card" style={{ borderRadius: 0, minHeight: 160, display: "flex", flexDirection: "column", gap: 10 }}>
                      <span className="number-label" style={{ color: flow.color }}>{s.n}</span>
                      <p style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "clamp(14px,2vw,17px)" }}>{s.title}</p>
                      <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.7, flex: 1 }}>{s.body}</p>
                    </div>
                  </FadeUp>
                ))}
                <FadeUp delay={flow.steps.length * 0.08}>
                  <div style={{ background: "var(--surface)", border: `1px solid ${flow.color === "var(--accent)" ? "rgba(200,240,96,0.15)" : flow.color === "var(--warning)" ? "rgba(245,166,35,0.15)" : "rgba(96,165,250,0.15)"}`, padding: 28, display: "flex", flexDirection: "column", justifyContent: "flex-end", minHeight: 160 }}>
                    <a href={flow.href} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", background: flow.color, color: "var(--bg)", fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase", padding: "13px 20px", borderRadius: 2, textDecoration: "none", transition: "opacity 0.2s" }}
      ...
      {/* PORTALS */}
      <section id="portals" style={{ borderTop: "1px solid var(--border)", padding: "clamp(56px,10vw,112px) clamp(16px,4vw,48px)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <FadeUp>
            <p className="number-label" style={{ color: "var(--accent)", marginBottom: 16 }}>The Portals</p>
            <h2 style={{ fontSize: "clamp(28px,5vw,56px)", fontWeight: 800, lineHeight: 1.05, marginBottom: 48, letterSpacing: "-0.03em" }}>Pick your portal.</h2>
          </FadeUp>
                ...
                <div style={{ background: "var(--surface)", padding: "clamp(20px,3vw,32px)", display: "flex", flexDirection: "column", gap: 20, height: "100%", position: "relative", overflow: "hidden" }}>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                      <p style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "clamp(20px,2.5vw,26px)", color: p.color, letterSpacing: "-0.02em" }}>{p.role}</p>
                    </div>
